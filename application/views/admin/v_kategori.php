    ...
    $('.help-block').empty(); // clear error string
        $.ajax({
            ...
            success: function(data)
            {
                ...
                $('.modal-title').text('Edit Kategory Produk'); // Set title to Bootstrap modal title
                if(data.gambar)
                {
                    $('#label-photo').text('Change Photo'); // label photo upload
                    $('#photo-preview div').html('<img src="<?= base_url('assets/assets/img/kategori/') ?>'+data.gambar+'" width="0" height"0" class="img-responsive">'); // show photo
